...
@pytest.fixture
def fake_yum_repos(request, target_sat, module_org, module_ak, module_cv, module_product):
    ...
    yield repositories.values()
    # cleanup: delete the created repositories,
    # first, remove environment and content-view associations from AK,
    # then, delete all of the published version(s) which may contain the repos,
...
def registered_contenthosts(request, target_sat, module_org, module_ak, module_cv, fake_yum_repos):
    ...
    example: No parametrization will result in 2 hosts of DEFAULT RHEL Version.
    ...
    assert all([isinstance(count, int), count > 0]), 'Count must be an integer greater than zero.'
    ...
    # Add the 3 yum repos to CV, update, needs_publish should be True
    module_cv.repository = [repo.read() for repo in fake_yum_repos]
