...
def module_target_sat_insights(request, module_target_sat, satellite_factory):
    """A module-level fixture to provide a Satellite host configured for Insights.
    By default, it returns the existing Satellite host provided by module_target_sat.

    If parametrized with a false value, then it will deploy a Satellite with
    ...
    if not hosted_insights:
        ...
        # FIXME Temporary script to generate and mount advisor rule content
        script = getattr(settings.rh_cloud, 'temp_iop_install_script', '').splitlines()
        for cmd in script:
...
def rhel_insights_vm(
    ...
    if module_target_sat_insights.local_advisor_enabled:
