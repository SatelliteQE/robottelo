...
def module_mcp_target_sat(module_target_sat):
    ...
    module_target_sat.execute(
        f'podman run --name {container_name} -d --pull=never -it -p {settings.foreman_mcp.port}:8080 foreman-mcp-server --foreman-url https://{settings.server.hostname} --foreman-username {settings.foreman_mcp.username} --foreman-password {settings.foreman_mcp.password} --host 0.0.0.0'
    )
    result = module_target_sat.execute(
        f'podman inspect -f "{{{{.State.Status}}}}" {container_name}'
    )
    assert result.stdout.strip() == 'running', f'failed to start container {container_name}'
    sleep(20)
