...
def module_mcp_target_sat(module_target_sat):
    ...
    result = module_target_sat.execute(
        f'podman pull {settings.foreman_mcp.registry}{settings.foreman_mcp.image_path}'
    )
    assert result.status == 0
    module_target_sat.execute(
        f'podman run --name {container_name} -d --pull=never -it -p {settings.foreman_mcp.port}:8080 \
            foreman-mcp-server --foreman-url https://{module_target_sat.hostname} --host 0.0.0.0 --no-verify-ssl'
    )
    sleep(20)
    result = module_target_sat.execute(
...
def module_downstream_mcp_target_sat(module_target_sat):
    ...
    container_name = f'mcp_server_downstream-{datetime.timestamp(datetime.now())}'
    result = module_target_sat.execute(
        f'firewall-cmd --permanent --add-port="{settings.foreman_mcp_downstream.port}/tcp"'
    )
    assert result.status == 0
    result = module_target_sat.execute('firewall-cmd --reload')
    assert result.status == 0
    module_target_sat.podman_login(
        settings.foreman_mcp_downstream.registry_stage_username,
        settings.foreman_mcp_downstream.registry_stage_password,
        settings.foreman_mcp_downstream.registry_stage,
    )
    result = module_target_sat.execute(
        f'podman pull {settings.foreman_mcp_downstream.registry_stage}{settings.foreman_mcp_downstream.image_path}'
    )
    assert result.status == 0
    module_target_sat.execute(
        f'podman run --name {container_name} -d --pull=never -it -p {settings.foreman_mcp_downstream.port}:8080 \
            foreman-mcp-server --foreman-url https://{module_target_sat.hostname} --host 0.0.0.0 --no-verify-ssl'
    )
    sleep(20)
    result = module_target_sat.execute(
