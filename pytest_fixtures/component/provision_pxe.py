...
def pxeless_static_discovery_host(
    ...
    result = sat.execute(
        'cd /var/www/html/pub && '
        f'discovery-remaster {module_discovery_sat.iso} '
        f'"proxy.type=foreman proxy.url=https://{sat.ip_addr}:443 fdi.pxmac={mac} fdi.pxauto=1 fdi.pxip={ip} fdi.pxgw={subnet.gateway} fdi.pxdns={subnet.dns_primary} fdi.pxmask={subnet.mask} fdi.pxfactname=discovery"'
    )
    pattern = re.compile(r"foreman-discovery-image\S+")
    fdi = pattern.findall(result.stdout)[0]
    Broker(
