...
def pxeless_static_discovery_host(
    ...
    ip = f"{subnet.from_}/{subnet.cidr}"
    subnet.ipam, subnet.boot_mode, subnet.dhcp = 'None', 'Static', ''
    ...
    result = sat.execute(
        'cd /var/www/html/pub && '
        f'discovery-remaster {module_discovery_sat.iso} '
        f'"proxy.type=foreman proxy.url=https://{sat.hostname}:443 fdi.pxmac={mac} fdi.pxauto=1 fdi.pxip={ip} fdi.pxgw={subnet.gateway} fdi.pxdns={subnet.dns_primary}"'
    )
    pattern = re.compile(r"foreman-discovery-image\S+")
    fdi = pattern.findall(result.stdout)[0]
    Broker(
