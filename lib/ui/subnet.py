...
class Subnet(Base):
    ...
    def remove(self, subnet_name, really):
        element = self.wait_until_element((locators["subnet.delete"][0], locators["subnet.delete"][1] % subnet_name), delay=5)
        if element:
            element.click()
            if really:
    ...
    def search(self, subnet_name):
        ...
        if searchbox:
            ...
            subnet_object = self.find_element((locators["subnet.display_name"][0], locators["subnet.display_name"][1] % subnet_name))
            if subnet_object:
                subnet_object.click()
                if (self.wait_until_element(locators["subnet.name"])):
                    result = dict([('name', None), ('network', None), ('mask', None)])
                    result['name'] = self.find_element(locators["subnet.name"]).get_attribute("value")
                    result['network'] = self.find_element(locators["subnet.network"]).get_attribute("value")
                    result['mask'] = self.find_element(locators["subnet.mask"]).get_attribute("value")
        return result

    def update(self, subnet_name, new_subnet_name=None, new_subnet_network=None, new_subnet_mask=None):
        if self.wait_until_element((locators["subnet.display_name"][0], locators["subnet.display_name"][1] % subnet_name), delay=2):
            subnet_object = self.find_element((locators["subnet.display_name"][0], locators["subnet.display_name"][1] % subnet_name))
        if subnet_object:
            subnet_object.click()
        if new_subnet_name:
