...
in upstream GitHub oull requests (PRs) and mapping them to test components via configured rules.
...
def component_match(item, components, base_marker):
    ...
    """
    # Check base marker requirement if specified
...
def pytest_collection_modifyitems(session, items, config):
    ...
    # Collect components from all specified PRs
    components = set()
    github_client = Github()

    for pr_info in upstream_prs:
        try:
            # Parse and validate PR format
            if '/' not in pr_info:
                raise ValueError(
            ...
            repo_config = settings.github_repos.get(repo_key)
            if not repo_config:
                available_repos = (
                    ', '.join(settings.github_repos.keys()) if settings.github_repos else 'none'
                )
                raise ValueError(
            ...
            try:
                ...
                if e.status == 404:
                    ...
                    logger.error(
                        "GitHub API rate limit or permission issue. Consider setting GITHUB_TOKEN."
                    )
                else:
                    logger.error(f"GitHub API error for {repo_key}/{pr_id}: {e}")

                # Option to continue with other PRs instead of failing completely
                if len(upstream_prs) > 1:
                    logger.warning("Continuing with remaining PRs...")
                    continue
                raise
    ...
    selected = []
    deselected = []
    base_marker = getattr(settings.github_repos, 'base_marker', None)
