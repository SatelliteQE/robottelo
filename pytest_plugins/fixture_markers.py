...
def pytest_generate_tests(metafunc):
    if 'rhel_contenthost' in metafunc.fixturenames:
        wf = settings.content_host.deploy_workflow
        rhel_parameters = [
            dict(workflow=wf, rhel_version=ver) for ver in settings.content_host.rhel_versions
        ]
        metafunc.parametrize(
            'rhel_contenthost',
            rhel_parameters,
            ids=[f'{wf}_rhel_{r["rhel_version"]}' for r in rhel_parameters],
            indirect=True,
        )
