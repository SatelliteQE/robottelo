...
def add_permission_to_user(ucre, perm):
    role = Role()
    ...
    ucre.role_ids = [created_role.id]
    ApiCrud.record_update(ucre)
...
class TestPermission(BaseAPI):
    ...
    def test_basic_create(self, test_data):
        ...
        udef = User()
        ucre = ApiCrud.record_create_recursive(udef)

        with self.assertRaises(Exception):
            ApiCrud.record_create(deps, udef)

        perm = ApiCrud.record_resolve(
            test_data._meta.api_class.permissions.create
            )
        add_permission_to_user(ucre, perm)
        ApiCrud.record_create(deps, udef)
    ...
    def test_basic_read(self, test_data):
        ...
        with self.assertRaises(Exception):
            ApiCrud.record_resolve(created, udef)

        perm = ApiCrud.record_resolve(
            test_data._meta.api_class.permissions.resolve
            )
        add_permission_to_user(ucre, perm)
        eres = ApiCrud.record_resolve(created, udef)
    ...
    def test_basic_remove(self, test_data):
        ...
        with self.assertRaises(Exception):
            ApiCrud.record_remove(created, udef)
    ...
    def test_basic_update(self, test_data):
        ...
        with self.assertRaises(Exception):
            ApiCrud.record_update(created, udef)

        perm = ApiCrud.record_resolve(
            test_data._meta.api_class.permissions.update
            )
        add_permission_to_user(ucre, perm)
        updated = ApiCrud.record_update(created, udef)
