...
def test_positive_get_count_for_host(
    ...
    assert (
        result.status == 0
    ), f'Failed to register the host - {rhel10_contenthost.hostname}: {result.stderr}'
    assert rhel10_contenthost.subscribed
    rhel10_contenthost.execute(r'subscription-manager repos --enable \*')
    host = rhel10_contenthost.nailgun_host.read()
    # No applicable errata to start
    assert rhel10_contenthost.applicable_errata_count == 0
    ...
    result = rhel10_contenthost.execute(f'yum install -y {FAKE_9_YUM_OUTDATED_PACKAGES[7]}')
    ...
    result = rhel10_contenthost.execute(f'yum install -y {FAKE_9_YUM_OUTDATED_PACKAGES[3]}')
    ...
    result = rhel10_contenthost.execute(f'yum install -y {FAKE_1_CUSTOM_PACKAGE}')
    ...
    assert rhel10_contenthost.applicable_errata_count == 4
...
def test_positive_get_applicable_for_host(
    ...
    for errata in REPO_WITH_ERRATA['errata']:
        ...
        result = rhel10_contenthost.execute(f'yum erase -y {package_name}')
        if result.status != 0:
            pytest.fail(f'Failed to remove {package_name}: {result.stdout} {result.stderr}')

    rhel10_contenthost.execute('subscription-manager repos')
    assert rhel10_contenthost.applicable_errata_count == 0
    host = rhel10_contenthost.nailgun_host.read()
    ...
    rhel10_contenthost.run(f'yum install -y {FAKE_1_CUSTOM_PACKAGE}')
...
def test_positive_incremental_update_required(
    ...
    assert result.status == 0, f'Failed to register the host: {rhel10_contenthost.hostname}'
    assert rhel10_contenthost.subscribed
    rhel10_contenthost.execute(r'subscription-manager repos --enable \*')
    host = rhel10_contenthost.nailgun_host.read()
...
def test_positive_incremental_update_apply_to_envs_cvs(
    ...
    assert rhel10_contenthost.execute(f'yum install -y {pkgs}').status == 0
    rhel10_contenthost.execute('subscription-manager repos')
    ...
    assert (
        response == []
    ), f'No incremental updates should currently be available to host: {rhel10_contenthost.hostname}.'
    ...
    rhel10_contenthost.execute('subscription-manager repos')
    host_app_errata = rhel10_contenthost.applicable_errata_count
    assert host_app_errata == 0
    # After adding filter to cv, check available incremental updates
    host_app_packages = rhel10_contenthost.applicable_package_count
    response = target_sat.api.Host().bulk_available_incremental_updates(
        data={
            'organization_id': module_sca_manifest_org.id,
            'included': {'ids': [host.id]},
    ...
    assert (
        response
    ), f'Expected one incremental update, but found none, for host: {rhel10_contenthost.hostname}.'
    # find that only expected CV version has incremental update available
    assert (
        len(response) == 1
    ), f'Incremental update should currently be available to only one host: {rhel10_contenthost.hostname}.'
    ...
    assert rhel10_contenthost.applicable_errata_count == host_app_errata + len(
        FAKE_9_YUM_SECURITY_ERRATUM
    )
    # newly added errata from incremental version are now applicable to host
    post_app_errata_ids = errata_id_set(
        _fetch_available_errata_instances(target_sat, rhel10_contenthost)
    )
    ...
    assert 8 == host_app_packages == rhel10_contenthost.applicable_package_count
    # install all of the newly added packages, recalculate applicability
    for pkg in added_packages:
        assert rhel10_contenthost.run(f'yum install -y {pkg}').status == 0
    rhel10_contenthost.execute('subscription-manager repos')
