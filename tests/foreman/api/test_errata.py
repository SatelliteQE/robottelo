...
def test_positive_incremental_update_apply_to_envs_cvs(
    ...
    assert rhel_contenthost.execute(f'yum install -y {pkgs}').status == 0
    rhel_contenthost.execute('subscription-manager repos')  # updates package profile
    ...
    assert response == [], (
        f'No incremental updates expected for the host: {rhel_contenthost.hostname}.'
    )
    ...
    assert len(response) == 1, (
        f'Incremental update should currently be available to only one host: {rhel_contenthost.hostname}.'
    )
    ...
    rhel_contenthost.execute('subscription-manager repos')  # update package profile
    ...
    assert rhel_contenthost.applicable_errata_count == len(FAKE_9_YUM_SECURITY_ERRATUM)
    # newly added errata from incremental version are now applicable to host
    post_app_errata_ids = errata_id_set(
        _fetch_available_errata_instances(target_sat, rhel_contenthost)
    )
    ...
    assert 8 == host_app_packages == rhel_contenthost.applicable_package_count
    # install all of the newly added packages, recalculate applicability
    assert rhel_contenthost.run(f'yum install -y {" ".join(added_packages)}').status == 0
    rhel_contenthost.execute('subscription-manager repos')  # update package profile
    ...
    assert response == [], (
        f'No incremental updates should currently be available to host: {rhel_contenthost.hostname}.'
    )
