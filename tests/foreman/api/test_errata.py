...
def _fetch_available_errata(host, expected_amount=None, timeout=120):
    ...
    errata = host.errata()
    for _ in range(timeout // 5):
        if expected_amount is None or len(errata['results']) == expected_amount:
...
def package_applicability_changed_as_expected(
    ...
    """Checks that after installing some package, updated any impacted errata(s)
    ...
    :return: result(boolean), or relevant applicables(dict)
        ...
        :return_applicables: if True: return dict of relevant applicable and changed entities:
    ...
    if prior_applicable_package_count == host.applicable_package_count:
        ...
        # no applicable errata were impaced by package install
        return False
    ...
    if app_errata_with_package_diff:
        if host.applicable_errata_count > prior_applicable_errata_count:
            ...
            raise AssertionError(
                ...
                f' {host.applicable_errata_count} were found, but expected {host.applicable_errata_count + len(app_errata_diff_ids)}.'
            )
    else:
...
def cv_publish_promote(sat, org, cv, lce=None, needs_publish=True):
    ...
    lce_ids = 'Library'
    if lce is not None:
        lce_ids = [lce.id] if not isinstance(lce, list) else [_lce.id for _lce in lce]
