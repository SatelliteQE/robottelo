...
def test_positive_applied_errata_by_install_date(
    ...
    today_utc = datetime.utcnow() - timedelta(minutes=15)
    ...
    report_today = rt.generate(
        data={
            ...
            'report_format': 'json',
            'input_values': {
                'Since': today_utc.strftime('%Y-%m-%d %H:%M:%S'),
                'Filter Errata Type': 'all',
                'Include Last Reboot': 'no',
                'Status': 'all',
            },
    ...
    assert all(
        errata_id in [entry['erratum_id'] for entry in report_today]  #
        for errata_id in ERRATUM_IDS
    )
    # timestamps are on or after today_utc
    assert all(
        today_utc <= datetime.strptime(entry['date'], '%Y-%m-%d %H:%M:%S')  #
        for entry in report_today
    )
    ...
    report_yesterday = rt.generate(
        data={
            ...
            'report_format': 'json',
            'input_values': {
                'Up to': yesterday_utc.strftime('%Y-%m-%d %H:%M:%S'),
                'Filter Errata Type': 'all',
                'Include Last Reboot': 'no',
                'Status': 'all',
            },
