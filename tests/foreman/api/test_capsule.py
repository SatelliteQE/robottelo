...
@pytest.mark.upgrade
def test_positive_update_capsule_hammer(target_sat, module_capsule_configured):
    ...
    result = module_capsule_configured.execute(
        'yum install rubygem-hammer_cli_katello rubygem-hammer_cli_foreman -y'
    )
    assert result.status == 0
    result = module_capsule_configured.execute('rpm -q "rubygem-hammer_cli"')
    assert result.status == 0
    ...
    job = target_sat.api.JobInvocation().run(
        ...
        data={
            ...
            'inputs': {
                'whitelist_options': 'check-non-redhat-repository,repositories-validate',
            },
