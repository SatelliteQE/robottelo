...
def _read_log(ch, pattern):
    ...
    while ch.recv_ready():
        ...
        while ch.recv_ready():
            char_encoded = ch.recv(1)
            try:
                char = char_encoded.decode('utf-8')
            except UnicodeEncodeError:
                logger.warning('Failed to decode char using utf-8 charset, replacing by "?"')
                char = '?'
            if char == '\n':
                break
            else:
...
def _assert_discovered_host(host, channel=None, user_config=None):
    ...
    for pattern in [
        (f"DHCPDISCOVER from {host.mac}", "DHCPDISCOVER",),
        (f"DHCPACK on [0-9.]+ to {host.mac}", "DHCPACK"),
    ]:
        try:
            dhcp_pxe = _wait_for_log(channel, pattern[0], timeout=30)
    ...
    for pattern in [
        ...
        try:
            _wait_for_log(channel, pattern[0], timeout=120)
        except TimedOutError:
            # raise assertion error
...
def test_positive_provision_pxe_host(_module_user, discovery_settings, provisioning_env):
    ...
    with ssh_client.invoke_shell() as channel:
        ...
        with LibvirtGuest() as pxe_host:
            ...
            assert provisioning_env['ptable']['name'] in [
                pt.read().name for pt in provisioned_host.operatingsystem.read().ptable
            ]
...
def test_positive_reboot_pxe_host(_module_user, discovery_settings, provisioning_env):
    ...
    with ssh_client.invoke_shell() as channel:
        ...
        with LibvirtGuest() as pxe_host:
            ...
            for pattern in [
                (f'DHCPDISCOVER from {pxe_host.mac}', 'DHCPDISCOVER',),
    ...
    with LibvirtGuest() as pxe_host_1:
        ...
        with LibvirtGuest() as pxe_host_2:
            ...
            for pxe_host in [(pxe_host_1, channel_1), (pxe_host_2, channel_2)]:
                for pattern in [
                    (f'DHCPDISCOVER from {pxe_host[0].mac}', 'DHCPDISCOVER',),
