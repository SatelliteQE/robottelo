...
class TestCapsuleContentManagement:
    ...
    @pytest.mark.tier4
    @pytest.mark.skip_if_not_set('capsule', 'clients', 'fake_manifest')
    def test_positive_capsule_sync(self, capsule_configured):
        ...
        repo_url = create_repo(repo_name, settings.repos.yum_1.url, constants.YUM_REPO_RPMS[0:2])
        ...
        lce_revision_capsule = get_repomd_revision(
            lce_repo_path, hostname=capsule_configured.ip_addr
        )

        assert get_repo_files(lce_repo_path, hostname=capsule_configured.ip_addr) == get_repo_files(
            cvv_repo_path
        )
        ...
        create_repo(repo_name, settings.repos.yum_1.url, constants.YUM_REPO_RPMS[-1:])
    ...
    def test_positive_mirror_on_sync(self, capsule_configured, rhel7_contenthost, default_sat):
        ...
        repo1_url = create_repo(repo1_name, settings.repos.yum_1.url, constants.YUM_REPO_RPMS[1:3])
