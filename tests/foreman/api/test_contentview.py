...
class TestRollingContentView:
    ...
    def test_negative_promote_rolling_version(
        ...
        def_rolling_cv.environment = [default_org.library.read()]
        with pytest.raises(HTTPError):
    ...
    def test_negative_rolling_in_a_composite(self, target_sat, module_org):
        ...
        with pytest.raises(TypeError):
            ...
            composite_cv = target_sat.api.ContentView(
                component=[rolling_cv.read()], organization=module_org, composite=True
            ).create()
        # try updating empty composite's components with the rolling cv
        composite_cv = target_sat.api.ContentView(organization=module_org, composite=True).create()
    ...
    def test_positive_host_with_rolling_content_source(
        self, rhel_contenthost, module_target_sat, module_sca_manifest_org, module_product
    ):
        ...
        :setup:
            ...
            4) Register a RHEL host to the activation key.
            5) SCA enabled, enable repositories from rolling cv.
        ...
        custom_repo = module_target_sat.api.Repository(
            product=module_product, url=settings.repos.yum_9.url
        ).create()
        custom_repo.sync()
        ...
        rolling_cv = module_target_sat.api.ContentView(organization=org, rolling=True).create()
        ...
        ak = module_target_sat.api.ActivationKey(
            organization=org,
            content_view=rolling_cv,
            environment=rolling_cv.environment[0].read(),
        ).create()
        # Hammer CLI: override the repos to enabled for AK
        override = module_target_sat.cli_factory.override_repos_for_activation_key(
        ...
        time.sleep(120)  # rh repos not reported immediately
        ...
        custom_repo_content_label = module_target_sat.cli.Repository.info({'id': custom_repo.id})[
            'content-label'
        ]
        ...
        rh_repo_content_label = module_target_sat.cli.Repository.info({'id': rh_repo.id})[
            'content-label'
        ]
        assert rh_repo_content_label in sub_man_repos
        # rh repo's package (python3) is installed and up to date
        ...
        # outdated package makes two errata installable
        assert client.applicable_errata_count
        assert client.applicable_package_count
        ...
        rolling_cv = rolling_cv.read()
        # try to delete rolling cv, delete ak and delete cv from Library first
        with pytest.raises(HTTPError):
            rolling_cv.delete()
        ak.delete()
        rolling_cv.delete_from_environment(rolling_cv.environment[0].id)
    ...
    @pytest.mark.stubbed
    @pytest.mark.e2e
    def test_positive_host_collection_with_rolling_content_source(self, content_hosts):
