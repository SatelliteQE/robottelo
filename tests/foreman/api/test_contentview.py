...
class TestContentViewPublishPromote:
    ...
    def test_inc_update_composite_updated_expected_version(
        ...
        :steps: GET content_view_versions/incremental_update <content_view_version_environments> <add_content> <propagate_all_composites>
            ...
            5. Publish the CompositeCV one final time (v2.0).
            6. Publish the Unfiltered ContentView again, v2.0 (child_cvs[0]).
        ...
        :expectedresults:
            ...
            2. CompositeCV contains all the latest CV Versions, except the Unfiltered CV v2.0 (only expect v1.0).
            3. After final publish, and publishing Unfiltered CV, CompositeCV now contains the Unfiltered CV v2.0.
        ...
        for comp in composite_cv.component:
            ...
            if cv_id == child_cvs[1].id:
                # just Incrementally Updated, filtered CV
        ...
        # Publish v2.0 of Unfiltered CV, after Composite v2.0.
        child_cvs[0].publish()
        child_cvs = [_cv.read() for _cv in child_cvs]
        child_versions = {_ver.read() for _cv in child_cvs for _ver in _cv.version}
        composite_cv = composite_cv.read()
        # Now expect Unfiltered CV v2.0 to be present in Composite
        for comp in composite_cv.component:
            comp = comp.read()
            assert comp.id in [version.id for version in child_versions]
            cv_id = comp.content_view.id
            if cv_id == child_cvs[0].id:
                # Unfiltered CV, latest v2.0 present
