...
class TestContentViewPublishPromote:
    ...
    def test_positive_inc_update_ccv_updated_expected_version(
        self,
        target_sat,
        module_org,
        fake_yum_repos,
    ):
        """Perform an Incremental Update, on a Version being used as the latest Version of a Content View,
        added to a Composite. The Composite View, has 3 regular ContentViews added, each containing a custom repo.
        Invoke the Incremental Update, to add an Erratum to the Composite's latest Version.
        ...
        :setup:
            ...
            2. Publish v1.0 for each of the 3 regular ContentViews (empty).
            3. Add a unique fake repo to each ContentView, option 'Update to Latest Version' to True.
        ...
        :steps:
            GET content_view_versions/incremental_update <content_view_version_environments> <add_content> <propagate_all_composites>
            1. Invoke the Incremental Update on a child-CV Version in Library, specifying erratum that was previously filtered out.
