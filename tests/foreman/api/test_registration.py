...
def test_positive_katello_ca_crt_refresh(
    ...
    ca_file_before = len(str(rhel_contenthost.execute('cat /etc/rhsm/ca/katello-server-ca.pem')))

    # corrupt the certificate file
    corrupt_data = gen_string('alphanumeric')
    result = rhel_contenthost.execute(
        f'sed -i "$ a {corrupt_data}" /etc/rhsm/ca/katello-server-ca.pem'
    )
    assert result.status == 0
    ca_file_after = len(str(rhel_contenthost.execute('cat /etc/rhsm/ca/katello-server-ca.pem')))
    ...
    ca_file_after_refresh = len(
        str(rhel_contenthost.execute('cat /etc/rhsm/ca/katello-server-ca.pem'))
    )
