...
class TestAnsibleREX:
    ...
    def test_positive_ansible_job_privilege_escalation(
        ...
        assert result.status == 0, f'Failed to register host: {result.stderr}'
        assert rhel_contenthost.execute('useradd rexuser').status == 0
        rex_passwd1 = rhel_contenthost.execute(
            f'echo {settings.server.ssh_password} | passwd rexuser --stdin'
        )
        assert 'passwd: all authentication tokens updated successfully.' in rex_passwd1.stdout
        pswd = rhel_contenthost.execute(
            'echo "rexuser   ALL=NOPASSWD:   ALL" | tee -a /etc/sudoers.d/rexuser'
        )
        assert pswd.status == 0
        assert rhel_contenthost.execute('useradd testing').status == 0
        rex_passwd2 = rhel_contenthost.execute(
            f'echo {settings.server.ssh_password} | passwd testing --stdin'
        )
        assert 'passwd: all authentication tokens updated successfully.' in rex_passwd2.stdout
        rhel_contenthost.execute('su - testing -c "whoami"')
