...
def test_positive_backup_restore(
    ...
    result = sat_maintain.execute(
        f"tar -tvf {backup_dir}/config_files.tar.gz | grep -E '{sat_maintain.hostname}|ansible.cfg'"
    ).stdout
    # Check if certificate tar file is present in Capsule backup.
    if instance == 'capsule':
        assert f'{sat_maintain.hostname}-certs.tar' in result
    # Check Ansible configuration file under /etc/ansible is present
    assert 'etc/ansible/ansible.cfg' in result
