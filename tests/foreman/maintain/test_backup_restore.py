...
REMOTEDB_SAT_FILES = ONLINE_SAT_FILES - {"pg_globals.dump"}
...
def get_exp_files(sat_maintain, backup_type, skip_pulp=False):
    if type(sat_maintain) is Satellite:
        ...
        if sat_maintain.is_remote_db():
            expected_files = REMOTEDB_SAT_FILES
        else:
            expected_files = ONLINE_SAT_FILES if backup_type == 'online' else OFFLINE_FILES
    else:
...
def test_positive_backup_offline_logical(sat_maintain, setup_backup_tests, module_synced_repos):
    ...
    if type(sat_maintain) is Satellite:
        if sat_maintain.is_remote_db():
            expected_files = REMOTEDB_SAT_FILES | CONTENT_FILES
        else:
