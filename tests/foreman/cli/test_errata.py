...
def test_negative_install_by_hc_id_without_errata_info(
    ...
    with pytest.raises(CLIReturnCodeError, match="Error: Option '--errata' is required"):
        HostCollection.erratum_install(
            {'id': host_collection['id'], 'organization-id': module_entitlement_manifest_org.id}
        )
...
def test_negative_install_by_hc_name_without_errata_info(
    ...
    with pytest.raises(CLIReturnCodeError, match="Error: Option '--errata' is required"):
        HostCollection.erratum_install(
            {'name': host_collection['name'], 'organization-id': module_entitlement_manifest_org.id}
        )
...
def test_positive_list_affected_chosts_by_erratum_restrict_flag(
    ...
    def cleanup():
        cv_filter_cleanup(
            cv_filter['filter-id'], module_cv, module_entitlement_manifest_org, module_lce
        )
...
def test_host_errata_search_commands(
    request, module_entitlement_manifest_org, module_cv, module_lce, host_collection, errata_hosts
):
...
def test_positive_list_filter_by_org_sort_by_date(
    ...
    """
    filter_sort_errata(
        module_entitlement_manifest_org, sort_by_date=sort_by_date, filter_by_org=filter_by_org
    )
...
def test_errata_list_by_contentview_filter(module_entitlement_manifest_org):
    ...
    errata_count = len(
        Erratum.list(
            {'organization-id': module_entitlement_manifest_org.id, 'content-view-id': cv.id}
        )
    )
