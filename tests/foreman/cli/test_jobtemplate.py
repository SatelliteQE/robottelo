...
def test_positive_job_template_crud(module_org, module_target_sat):
    ...
    job = module_target_sat.cli.JobTemplate.info({'name': template_name})
...
def test_positive_clone_job_template(module_org, module_target_sat):
    ...
    :steps:
        ...
        3. Delete the cloned job template
        4. Delete the original job template
    ...
    assert module_target_sat.cli.JobTemplate.info({'name': clone_name}) is not None
