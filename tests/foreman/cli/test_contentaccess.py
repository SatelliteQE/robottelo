...
def test_negative_rct_not_shows_golden_ticket_enabled(
    ...
    target_sat.upload_manifest(function_org.id, function_entitlement_manifest, interface='CLI')
    result = target_sat.execute(f'rct cat-manifest {function_entitlement_manifest.name}')
    manifest = clone()
    target_sat.upload_manifest(function_org.id, manifest, interface='CLI')
