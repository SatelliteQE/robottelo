...
@pytest.mark.tier4
@pytest.mark.stubbed
@pytest.mark.skip_if_open("BZ:1732522")
def test_positive_logging_verify_pulp_logs():
    """
    Verify Pulp logs are getting captured using pulp correlation ID
    ...
    :steps:
        ...
        4. Use Hammer command to get the UUID of Repository sync(label: Actions::Katello::Repository::Sync).
        5. Search for the UUID in production log and get the correaltion ID of pulpcore from it
        6. Search for that correlation ID in var/log/messages.

    :expectedresults:
        1.Pulp logs are being captured in the logs with the correlation ID.
