...
def test_host_facts_upload_on_host_registration(
    ...
    with module_target_sat.ui_session() as session:
        ...
        host_info = session.host.get_details(module_target_sat.hostname)
        assert 'Facts' in host_info['facts_details']


@pytest.mark.stream
def test_positive_auto_activation_select_for_single_ak(
    module_target_sat, rhel8_contenthost, activation_key, function_entitlement_manifest_org
):
    """Activation key should be auto-selected while registering host to a satellite
    when only one ak is present.

    :id: ceb623eb-303d-418e-8141-d2d2c41c5d37

    :CaseLevel: Integration

    :steps:
        1. Create an activation key.
        2. Try to register a host to satellite with activation key created in Step 1
        using global registration form.

    :expectedresults: Activation key is auto selected if there is only one activation key
    is present on satellite.
    ...
    with module_target_sat.ui_session() as session:
        cmd = session.host.get_register_command(
            {
                'general.organization': function_entitlement_manifest_org.name,
                'general.insecure': True,
                'advanced.force': True,
            }
        )
        client.execute(cmd)
        assert activation_key in cmd['general']['activation_keys']

        new_ak = module_target_sat.api.ActivationKey(
        ...
        client.execute(cmd)
        assert new_ak in cmd['general']['activation_keys']
