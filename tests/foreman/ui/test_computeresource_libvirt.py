...
def test_positive_associate_with_custom_profile(
    ...
    with session:
        ...
        assert provider_content['storage'][0]['storage_type'] == "QCOW2"
        assert provider_content['storage'][0]['storage_pool'] == "default"
        assert provider_content['storage'][0]['size'] == storage_data['size']
        assert provider_content['network_interfaces'][0]['network_type'] == "Physical (Bridge)"
        assert provider_content['network_interfaces'][0]['network'] == 'br-420'
        assert provider_content['network_interfaces'][0]['nic_type'] == "virtio"
