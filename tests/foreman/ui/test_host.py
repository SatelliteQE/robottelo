...
def test_positive_host_role_information(target_sat):
    ...
    location = (
        target_sat.api.Location().search(query={'search': f'id = {host.location.id}'})[0].name
    )
    organization = (
        target_sat.api.Organization()
        .search(query={'search': f'id = {host.organization.id}'})[0]
        .name
    )
    ...
    with target_sat.ui_session() as session:
        ...
        session.organization.select(organization)
