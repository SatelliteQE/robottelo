...
@pytest.mark.parametrize(
    ...
    [  # one test param for each supported RHEL major os
        pytest.param(
            {'rhel': rhel_ver, 'count': 4},
            id=f'rhel{rhel_ver}_4reg_chosts',
        )
...
def test_positive_apply_for_all_hosts(
    ...
    with session:
        # possible in-progress applicability task(s), 10s margin
        now = datetime.now(UTC).replace(microsecond=0)
        timestamp = (now - timedelta(seconds=10)).strftime(TIMESTAMP_FMT)
        session.location.select(loc_name=DEFAULT_LOC)
