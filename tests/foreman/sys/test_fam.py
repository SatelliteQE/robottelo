...
def setup_fam(module_target_sat, module_sca_manifest, install_import_ansible_role):
    ...
    module_target_sat.execute(f"sed -i '/test_crud/ s/pytest -vv/$(PYTEST_COMMAND)/' {FAM_ROOT_DIR}/Makefile")
...
def test_positive_run_modules_and_roles(module_target_sat, setup_fam, ansible_module):
    ...
    result = module_target_sat.execute(
        f'export NO_COLOR=True && cd {FAM_ROOT_DIR} && make livetest_{ansible_module} PYTHON_COMMAND="python3" PYTEST_COMMAND="pytest-3.11 -vv"'
    )
