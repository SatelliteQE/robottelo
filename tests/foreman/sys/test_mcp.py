...
async def test_positive_call_mcp_server(request, mcp_server):
    ...
    settings_obj = (
        settings.foreman_mcp_downstream
        if mcp_server == 'module_downstream_mcp_target_sat'
        else settings.foreman_mcp
    )
    async with Client(
        transport=StreamableHttpTransport(
            f'http://{target_sat.hostname}:{settings_obj.port}/mcp',
            headers={
                'FOREMAN_USERNAME': settings_obj.username,
                'FOREMAN_TOKEN': settings_obj.password,
            },
        ),
    ) as client:
