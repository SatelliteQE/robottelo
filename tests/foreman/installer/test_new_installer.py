...
pytestmark = [pytest.mark.new_installer, pytest.mark.build_sanity, pytest.mark.upgrade]
...
def test_positive_check_installer_hammer_ping(module_sat_ready_rhel):
    ...
    """
    assert module_sat_ready_rhel.execute('foremanctl setup-hammer').status == 0
