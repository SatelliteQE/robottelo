...
def test_positive_sync_without_deadlock(
    ...
    rh_repo_id = target_sat.api_factory.enable_rhrepo_and_fetchid(
        ...
        org_id=org.id,
        product=constants.REPOS['rhel8_aps']['product'],
        repo=constants.REPOS['rhel8_aps']['name'],
        reposet=constants.REPOSET['rhel8_aps'],
        releasever=constants.REPOS['rhel8_aps']['releasever'],
    )
    ...
    # Synchronize repository, Capsule Sync task triggered
    ...
    # within 240 seconds of end of repo_sync
    timestamp = datetime.utcnow().replace(microsecond=0) - timedelta(seconds=240)
    ...
    # capsule content reflects expected counts, from synced rh_repo in CVV
    repo = repo.read()
