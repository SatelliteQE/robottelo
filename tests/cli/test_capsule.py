...
class TestCapsule(BaseCLI):
    ...
    def subscribe_content_through_capsule(self):
        ...
        capsule
        @Setup: Content source types configured/synced for
        [RH, Custom, Puppet, Docker] etc.
        @Steps:
    ...
    def register_via_capsule_with_parent_cert(self):
        """
        @Feature: Capsules
        @Test: system can register via capsule using same cert as
        parent sat.  Note: it is possible that some changes may
        have to be made to rhsm.conf to point to satellite.  How
        this will be impl. has not yet been determined.  It will need
        its own case eventually.
        ...
        @Assert: Registration works with no changes, and certs RPM
        installed from parent satellite.
        @Status: Manual
        """
