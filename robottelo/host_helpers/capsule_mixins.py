...
class CapsuleInfo:
    ...
    def cutoff_host_setup_log(self, proxy_hostname, hostname):
        ...
        if settings.server.is_ipv6:
            result = self.execute(f'dig +short AAAA {hostname}')
            assert len(result.stdout.strip()) > 0, f'DNS record not found for {hostname}'
            assert (
                self.execute(
                    f'firewall-cmd --permanent --direct --add-rule ipv6 filter OUTPUT 1 -d {result.stdout.strip()} -j REJECT && firewall-cmd --reload'
                ).status
                == 0
            )
        else:
            result = self.execute(f'dig +short A {hostname}')
            assert len(result.stdout.strip()) > 0, f'DNS record not found for {hostname}'
            assert (
                self.execute(
                    f'firewall-cmd --permanent --direct --add-rule ipv4 filter OUTPUT 1 -d ${result.stdout.strip()} -j REJECT && firewall-cmd --reload'
                ).status
                == 0
            )
    ...
    def restore_host_check_log(self, proxy_hostname, hostname, old_log):
        ...
        if settings.server.is_ipv6:
            self.execute(
                f'firewall-cmd --permanent --direct --remove-rule ipv6 filter OUTPUT 1 -d $(dig +short AAAA {hostname}) -j REJECT && firewall-cmd --reload'
            )
        else:
            self.execute(
        ...
        satellite_ip = ssh.command(
            f'dig {"AAAA" if settings.server.is_ipv6 else "A"} +short $(hostname)'
        ).stdout.strip()
