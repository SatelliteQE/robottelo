...
class ClientsSettings(FeatureSettings):
    ...
    def read(self, reader):
        ...
        self.distros = [
            x.strip() for x in reader.get("clients", "distros", "rhel7").split(",")
        ]
...
class SSHClientSettings(FeatureSettings):
    ...
    def connection_timeout(self):
        return (
            self._connection_timeout if (self._connection_timeout is not None) else 10
        )
...
class UpgradeSettings(FeatureSettings):
    ...
    def validate(self):
        ...
        if self.rhev_cap_host and self.capsule_hostname:
            validation_errors.append(
                "Provide either rhev_cap_host or capsule_hostname, not both"
            )
        if self.rhev_capsule_ak and self.capsule_ak:
            validation_errors.append(
                "Provide either rhev_capsule_ak or capsule_ak, not both"
            )
        return validation_errors
...
class VirtWhoSettings(FeatureSettings):
    ...
    def validate(self):
        ...
        if self.hypervisor_type not in supported_hypervisors:
            validation_errors.append(
                "[virtwho] hypervisor_type must be one of {}".format(
...
class Settings(object):
    ...
    def _validate_robottelo_settings(self):
        ...
        if self.webdriver not in webdrivers:
            validation_errors.append(
                "[robottelo] webdriver should be one of {0}.".format(
                    ", ".join(webdrivers)
                )
            )
