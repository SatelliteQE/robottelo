...
class OperatingSys(Base):
    ...
    def update(self, os_name, new_name=None,
        ...
        element = self.wait_until_element((locators
                                           ["operatingsys.operatingsys_name"][0],
                                           locators
                                           ["operatingsys.operatingsys_name"][1]
                                           % os_name))
        if element:
            element.click()
        if new_name:
        ...
        if new_major_version:
            if self.wait_until_element(locators["operatingsys.major_version"]):
                self.field_update("operatingsys.major_version", new_major_version)
        if new_minor_version:
            if self.wait_until_element(locators["operatingsys.minor_version"]):
                self.field_update("operatingsys.minor_version", new_minor_version)
        if new_os_family:
            Select(self.find_element(locators
                                     ["operatingsys.family"]
