...
class Base(object):
    ...
    def _search_locator(self):
        """Specify element name locator which should be use in search procedure

        """
        raise NotImplementedError(
            'Subclasses must return locator of element to search')

    def _search_init(self, search_key=None, is_katello=None, button_timeout=15,
                     result_timeout=15):
        """Initialize every optional parameter for search procedure which can
        be overridden for any unique case"""
        if self.is_entity_delete:
            button_timeout = 3
            result_timeout = 1
        return {
            'search_key': search_key,
            'is_katello': is_katello,
    ...
    def search(self, element_name):
        ...
        optional_parameters = self._search_init()
        search_key = optional_parameters['search_key'] or 'name'
        is_katello = optional_parameters['is_katello']
        element_locator = self._search_locator()

        # Determine search box and search button locators depending on the type
        # of entity
        prefix = 'kt_' if is_katello else ''
        searchbox = self.wait_until_element(
            common_locators[prefix + 'search'],
            timeout=optional_parameters['button_timeout']
        )
        ...
        element = self.wait_until_element(
            ...
            timeout=optional_parameters['result_timeout'],
        )
        return element
