...
class ContentHost(Host, ContentHostMixins):
    ...
    def identity(self):
        ...
        if id_output:
            ...
            regged_to_output = self.execute('subscription-manager config | grep " hostname"').stdout
            if regged_to_output and len(regged_to_output.split('= ')) > 1:
                id_dict['registered_to'] = regged_to_output.replace('\n', '').split('= ')[1]
        return id_dict
