...
class Satellite(Capsule, SatelliteMixins):
    ...
    def get_foreman_raw_ca(self, verify=None, timeout=60):
        ...
        """
        url = urljoin(f'{self.url}/', 'unattended/public/foreman_raw_ca')
        try:
            response = requests.get(url, verify=verify, timeout=timeout)
            response.raise_for_status()
        except requests.RequestException as err:
            raise SatelliteHostError(f'Failed to fetch foreman_raw_ca from {url}: {err}') from err
    ...
    def get_server_certificate_from_ssl_socket(self, timeout=60):
        ...
        try:
            ...
            raise SatelliteHostError(
                f'Failed to connect to {host}:{port} for TLS server certificate: {err}'
            ) from err
        except ssl.SSLError as err:
            raise SatelliteHostError(
                f'TLS handshake failed retrieving server certificate from {host}:{port}: {err}'
            ) from err
        if not der:
            raise SatelliteHostError(f'No peer certificate presented by {host}:{port}')
        pem = ssl.DER_cert_to_PEM_cert(der)
        return pem if isinstance(pem, str) else pem.decode()
