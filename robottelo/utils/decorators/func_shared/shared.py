...
class _SharedFunction:
    ...
    def _has_result_expired(self, creation_datetime):
        ...
        return datetime.datetime.now(datetime.timezone.utc) >= expire_datetime
    ...
    def __call__(self):
        ...
        with self.storage.lock(self.key) as data:
            ...
            if call_function is True:
                ...
                creation_datetime = datetime.datetime.now(datetime.timezone.utc).strftime(
                    _DATETIME_FORMAT
                )
                if exp:
        ...
        if not call_function and error:
            ...
            raise SharedFunctionException(
                f'Error generated by process: {pid} Exception: {error_class_name} error: {error}'
            )
