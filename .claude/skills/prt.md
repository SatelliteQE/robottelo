...
`https://<JENKINS_HOSTNAME>/job/robottelo-pr-testing/<BUILD_NUMBER>/`
...
- **Jenkins MCP**: if not configured, run `claude mcp add jenkins-satqe https://<JENKINS_HOSTNAME>/mcp-server/mcp --transport http --header "Authorization: Basic <base64>"` then relaunch with `NODE_TLS_REJECT_UNAUTHORIZED=0 claude`.
